...
namespace TiltBrush
{
    ...
    public class GifEncodeTask
    {
        ...
        /// TODO(pld): when frameWidth == 500 and frameHeight == 500, the palettes seem to be
        /// created incorrectly.
        ...
        private void RunLow()
        {
            ...
            using (var encoder = new AnimatedGifEncoder32())
            {
                encoder.Start(m_GifName);
                encoder.SetSize(m_GifWidth, m_GifHeight);
                encoder.SetTransparent(new Color32(0, 0, 0, 255), false);
                encoder.SetDelay(m_FrameDelayMs);
                encoder.SetRepeat(0);  // infinite repeat
                encoder.DitherStrength = m_DitherStrength;
                ...
                if (!m_bPalettePerFrame)
                {
                    ...
                    for (int i = 0; i < m_Frames.Count; ++i)
                    {
                        encoder.PreAnalyzeFrame(m_Frames[i], m_GifWidth, m_GifHeight);
                ...
                for (int i = 0; i < m_Frames.Count; ++i)
                {
                    encoder.AddFrame(m_Frames[i], m_GifWidth, m_GifHeight);
                    m_CreationPercent = (float)(++nAdded) / nTotal;
                }
